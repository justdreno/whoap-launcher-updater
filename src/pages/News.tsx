...
import { Newspaper, GitBranch, WifiOff, Gamepad2, ExternalLink } from 'lucide-react';
        ...
        const loadContent = async () => {
            try {
                const [newsResult, changelogResult, minecraftNewsResult] = await Promise.all([
                    ...
                    ContentManager.fetchMinecraftNews()
                ]);
                setNews(newsResult.items);
                setChangelogs(changelogResult.items);
                setMinecraftNews(minecraftNewsResult.items);
                setFromCache(newsResult.fromCache || changelogResult.fromCache || minecraftNewsResult.fromCache);
            } catch (e) {
